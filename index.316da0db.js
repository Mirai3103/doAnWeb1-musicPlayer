...
let renctlyPlayed = localStorage.getItem("recentlyPlayed" + user.id) ? JSON.parse(localStorage.getItem("recentlyPlayed" + user.id)) : [];
  ...
  const element = createElementFromHTML(`
    ...
    <div class="flex flex-col py-2">
        <div class="p-2 py-4 flex gap-x-2 items-cente hover:bg-[#8f49f2] cursor-pointer toggle-favorive">
            ${isfavorite ? `<i class="fa-solid fa fa-heart"></i> <span>Xo\xE1 kh\u1ECFi y\xEAu th\xEDch</span>` : `<i class="fa-regular fa-heart"></i> <span>Th\xEAm v\xE0o y\xEAu th\xEDch</span>`} 
        </div>
...
const renderTrendingSong = async (time = null, audioPlayer2 = null) => {
  ...
  songs.forEach((song, index) => {
    ...
    const htmlString = `<div songId="${song.id}" class="grid grid-cols-8 font-normal text-sm hover:text-[#8338ec] ${currentTrackId === song.id ? "bg-slate-50 text-[#8338ec]" : ""} hover:bg-slate-50 py-1 cursor-pointer"
                                        >
                                            <div class="col-span-1 grid place-content-center ">${currentTrackId === song.id ? `<div class="music-waves -ml-2 -mb-2">
                                                            <span></span>
                                                            <span></span>
                                                            <span></span>
                                                            <span></span>
                                                            <span></span>
                                                        </div>` : formatNumber(index + 1, 2)}</div>
                                            <abbr title="${song.name}" class="col-span-3 flex items-center gap-x-4 px-2 play-song-btn">
                                                <div
      ...
      song.totalTime
                                            ...
                                            <div class="col-span-1 grid place-content-center ">
                                                <i class="fa-solid ${currentTrackId === song.id && (audioPlayer2 == null ? void 0 : audioPlayer2.isPlaying) ? "fa-pause" : "fa-play"}"></i>
                                            </div>
                                            <abbr title="Add to your favorite" class="add-favorite-btn col-span-1 grid place-content-center hover:text-red-500">
                                               ${isFavoriteSong ? `<i class="fa-solid fa-heart"></i>` : `<i class="fa-regular fa-heart"></i>`}
                                            </abbr>
                                            <div class="col-span-1 grid place-content-center">
                                               <a href="${song.url}" download target="_blank"> <i class="fa-solid fa-download"></i></a>
                                            </div>
                                        </div>
                                        `;
...
const renderQueueView = (audioPlayer2) => {
  ...
  const nowPlayingElement = createElementFromHTML(` <div class="grid grid-cols-8 text-[#8338ec] bg-slate-300 p-2 rounded-sm">
  ...
  for (let i = 0; i < remainingTracks.length; i++) {
    ...
    const nextSongElement = createElementFromHTML(` <div class="grid grid-cols-8 group hover:text-[#8338ec] hover:bg-slate-300 p-2 rounded-sm border-b">
                            <div class="col-span-6 flex items-center gap-x-4">
                                <div
...
const initQueueView = (audioPlayer2) => {
  ...
  queueBtn == null ? void 0 : queueBtn.addEventListener("click", () => {
  ...
  recentlyBtn == null ? void 0 : recentlyBtn.addEventListener("click", () => {
  ...
  async renderFavoriteSong() {
    ...
    for (const song of favoriteSongsList) {
      ...
      const element = createElementFromHTML(`
         ...
         <div class="basis-7 flex items-center ml-2">${audioPlayer.getCurrentTrack().id === track.id ? `<div class="music-waves -ml-2 -mb-2">
                                                            <span></span>
                                                            <span></span>
                                                            <span></span>
                                                            <span></span>
                                                            <span></span>
                                                        </div>` : formatNumber(++i + 1, 2)}</div>
                                        <div class="basis-[550px] flex items-center gap-x-4 px-2 cursor-pointer">
                                            ...
                                            <div
                                                class="font-bold text-ellipsis whitespace-nowrap overflow-hidden flex flex-col"
                                            >
                                                <span class="overflow-hidden max-w-[400px] text-ellipsis whitespace-nowrap">${track.name}</span>
                                            </div>
                                        </div>
                                        <div class="basis-[480px] flex items-center text-ellipsis whitespace-nowrap overflow-hidden">${track.artistName}</div>
...
const renderRecentlyAdded = async (audioPlayer2 = null) => {
  ...
  for (let i = 0; i < songs.length; i++) {
    ...
    const songElement = createElementFromHTML(`
            <div class="grid grid-cols-10 font-normal text-sm hover:text-[#8338ec] ${currentTrackId === song.id ? "bg-slate-50 text-[#8338ec]" : ""} hover:bg-slate-50 py-1 cursor-pointer">
                <div class="col-span-1 grid place-content-center ">${currentTrackId === song.id ? `<div class="music-waves -ml-2 -mb-2">
                                <span></span>
                                <span></span>
                                <span></span>
                ...
                <div class="col-span-3 grid place-content-center text-ellipsis whitespace-nowrap overflow-hidden">${song.artistName}</div>
            </div>
        `);
...
audioPlayer.addTrack({
  ...
  thumbnailUrl: "https://photo-resize-zmp3.zmdcdn.me/w240_r1x1_jpeg/covers/f/f/ffd96dc18d252825cb591e255ddc6dbd_1513860881.jpg",
  artistImgUrl: "https://photo-resize-zmp3.zmdcdn.me/w360_r1x1_jpeg/avatars/1/6/a/d/16ad38a571e873f840bbfc0d97214baa.jpg",
  totalTime: 310008
});
...
const handleHashChange = async (e) => {
  ...
  switch (page) {
    case "#favorite": {
      const album = {
        imageCover: "https://res.cloudinary.com/dkvga054t/image/upload/v1669367712/pngtree-vinyl-line-icon-heart-record-png-image_8230774-removebg-preview_haoqpu.png",
    ...
    case "#artist": {
      ...
      const listSongOfArtist = (await getListTrack()).filter((t) => t.artistName.toLowerCase().includes(artist.name.toLowerCase())).map((t) => t.id + "");
      const album = {
